...
congruentChars.forEach((observedCharacters, position) => {
   ...
   // console.log('Checking probability distribution of remaining keys...');
    ...
    key[position].forEach(testChar => {
        ...
        var probabilities = counters.map(n => n / lowercaseCongruentLetters.length);

        sums[testChar] = probabilities.map(c=>c).reduce((m,p)=>m+(p*p), 0);
    });

    var mostLikelyOffsets = sums.slice(0).sort((a,b)=>a-b).slice(0,5).map(sum => sums.indexOf(sum));

    console.log("\nMost likely offsets for position " + position + ":");
    mostLikelyOffsets.forEach(offset => {
       console.log(offset + " (0x" + offset.toString(16).toUpperCase() +  ") (sum: " + sums[offset] + ")")
    });

    key[position] = key[position].filter(c => mostLikelyOffsets.indexOf(c) !== -1);
});
